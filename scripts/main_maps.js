...
function drawGateLegend() {
  ...
  legendSvg
    ...
    .append("xhtml:div")
    .attr("font-size", "16px")
    .html(`<input id="constant_scale" checked type="checkbox" value="constant_scale" name="constant_scale"/><label for="constant_scale"> Use Constant Scale</label>`);
...
function updateMapScale() {
  ...
  if (
    filteredCarRouteData == undefined ||
    filteredCarRouteData == [] ||
    filteredCarRouteData === null ||
    filteredCarRouteData.length == 0
  ) {
    return;
  }
...
function renderSpatialMap(car_route_data, selectedPoint) {
  ...
  let max = d3.select("#constant_scale").property("checked") ?  max_count : maxValue ;

  let radiusScale = d3.scaleLinear().domain([0, max]).range([15, 90]);
  colorScale = d3.scaleSequential(mapDensityColorScheme).domain([0, max]);
  updateMapScale();
